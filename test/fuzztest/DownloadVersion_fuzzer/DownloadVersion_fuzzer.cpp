...
#include "DownloadVersion_fuzzer.h"
...
namespace OHOS {
    ...
    int32_t FuzzUpdateServiceDownloadVersionImpl(UpdateContext &ctx, UpdateCallbackInfo &cb,
        UpdatePolicy &policy, VersionInfo &versionInfo, int32_t service, UpgradeInfo &info)
    {
        ...
        int32_t ret = UpdateServiceKits::GetInstance().DownloadVersion();
    ...
    bool FuzzUpdateServiceDownloadVersion(const uint8_t* data, size_t size)
    {
        ...
        return !FuzzUpdateServiceDownloadVersionImpl(ctx, cb, policy, versionInfo, service, info);
    }
}
