...
#include "GetNewVersion_fuzzer.h"
...
namespace OHOS {
    ...
    int32_t FuzzUpdateServiceGetNewVersionImpl(UpdateContext &ctx, UpdateCallbackInfo &cb,
        UpdatePolicy &policy, VersionInfo &versionInfo, int32_t service, UpgradeInfo &info)
    {
        ...
        int32_t ret = UpdateServiceKits::GetInstance().GetNewVersion(versionInfo);
    ...
    bool FuzzUpdateServiceGetNewVersion(const uint8_t* data, size_t size)
    {
        ...
        return !FuzzUpdateServiceGetNewVersionImpl(ctx, cb, policy, versionInfo, service, info);
    }
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::FuzzUpdateServiceGetNewVersion(data, size);
    return 0;
}
