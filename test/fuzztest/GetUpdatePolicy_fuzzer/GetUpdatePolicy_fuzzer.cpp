...
#include "GetUpdatePolicy_fuzzer.h"
...
namespace OHOS {
    ...
    int32_t FuzzUpdateServiceGetUpdatePolicyImpl(UpdateContext &ctx, UpdateCallbackInfo &cb,
        UpdatePolicy &policy, VersionInfo &versionInfo, int32_t service, UpgradeInfo &info)
    {
        ...
        int32_t ret = UpdateServiceKits::GetInstance().GetUpdatePolicy(policy);
    ...
    bool FuzzUpdateServiceGetUpdatePolicy(const uint8_t* data, size_t size)
    {
        ...
        return !FuzzUpdateServiceGetUpdatePolicyImpl(ctx, cb, policy, versionInfo, service, info);
    }
}
