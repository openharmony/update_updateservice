...
#include "SetUpdatePolicy_fuzzer.h"
...
namespace OHOS {
    ...
    int32_t FuzzUpdateServiceSetUpdatePolicyImpl(UpdateContext &ctx, UpdateCallbackInfo &cb,
        UpdatePolicy &policy, VersionInfo &versionInfo, int32_t service, UpgradeInfo &info)
    {
        ...
        int32_t ret = UpdateServiceKits::GetInstance().SetUpdatePolicy(policy);
    ...
    bool FuzzUpdateServiceSetUpdatePolicy(const uint8_t* data, size_t size)
    {
        ...
        return !FuzzUpdateServiceSetUpdatePolicyImpl(ctx, cb, policy, versionInfo, service, info);
    }
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::FuzzUpdateServiceSetUpdatePolicy(data, size);
    return 0;
}
