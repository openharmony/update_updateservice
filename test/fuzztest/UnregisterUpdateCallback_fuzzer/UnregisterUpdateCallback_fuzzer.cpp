...
#include "UnregisterUpdateCallback_fuzzer.h"
...
namespace OHOS {
    ...
    int32_t FuzzUpdateServiceUnregisterUpdateCallbackImpl(UpdateContext &ctx, UpdateCallbackInfo &cb,
    ...
    bool FuzzUpdateServiceUnregisterUpdateCallback(const uint8_t* data, size_t size)
    {
        ...
        return !FuzzUpdateServiceUnregisterUpdateCallbackImpl(ctx, cb, policy, versionInfo, service, info);
    }
}
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::FuzzUpdateServiceUnregisterUpdateCallback(data, size);
    return 0;
}
