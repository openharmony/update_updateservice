...
#include "updateservicesetupdatepolicy_fuzzer.h"
...
namespace OHOS {
    ...
    int32_t FuzzUpdateServiceSetUpdatePolicyImpl(const uint8_t* data, size_t size)
    {
        UpdateContext ctx;
    ...
    bool FuzzUpdateServiceSetUpdatePolicy(const uint8_t* data, size_t size)
    {
        ...
        return !FuzzUpdateServiceSetUpdatePolicyImpl(data, size);
    }
}
