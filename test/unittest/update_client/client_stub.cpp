    ...
    void **data)               // [out] Receives the data pointer for the callback.
{
    ...
    switch (testEnv->testStage) {
        case (int32_t)SessionType::SESSION_GET_UPDATER: {
            if (*argc > 1) {
        ...
        case (int32_t)SessionType::SESSION_SET_POLICY: {
        ...
        case (int32_t)SessionType::SESSION_SUBSCRIBE:
        case (int32_t)SessionType::SESSION_UNSUBSCRIBE: {
            argv[0] = CreateNapiValue(napi_string, testEnv->eventType.c_str());
            if (*argc > 1) {
                argv[1] = CreateNapiValue(napi_function, nullptr);
            }
